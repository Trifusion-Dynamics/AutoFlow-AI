...
export class UsersController {
  async getProfile(req, res, next) {
    try {
      ...
      return successResponse(res, responseTransformer.user(user));
    } catch (error) {
      next(error);
    }
  }
  ...
  async updateProfile(req, res, next) {
    try {
      ...
      return successResponse(res, responseTransformer.user(updatedUser));
    } catch (error) {
      next(error);
    }
  }
  ...
  async getOrgMembers(req, res, next) {
    try {
      ...
      return successResponse(res, users.map(u => responseTransformer.user(u)), 200, { pagination: paginationMeta });
    } catch (error) {
      next(error);
    }
  }
  ...
  async removeMember(req, res, next) {
    try {
      ...
      return successResponse(res, responseTransformer.user(deletedUser));
    } catch (error) {
      next(error);
    }
  }
